...
import edu.utdallas.hltri.inquire.lucene.PseudoRelevanceFeedbackExpander;
...
public class CohortL2rProcessor<
  ...
  public void process(List<Document<Topic>> topics,
    ...
    expand(expanders, queries, topics, queryRepresentations, UMLSManager::new, "E1:UMLS");
    try (SNOMEDManager snomed = new SNOMEDManager()) {
      expand(expanders, queries, topics, queryRepresentations,  () -> snomed.expandBy(
          ...
          SNOMEDRelationshipDirection.CHILDREN), "E2:SNOMED");
    }

    expand(expanders, queries, topics, queryRepresentations,
        WikiRedirectManager::new, "QE3:Wiki");

    expand(expanders, queries, topics, queryRepresentations, UMLSManager::new, "E4:PRF");
    ...
    for (Document<Topic> topic : topics) {
      for (Entry<String, Function<Document<Topic>, BasicQuery>> qc :
        ...
        final Triple<String, String, String> key = Triple.of(topic.getId(), qc.getKey(), "E5:ALL");
        queries.put(key, qc.getValue().apply(topic).expand(e));
        topicQueryKeys.put(topic.getId(), key);
      }
    }

//    try (PseudoRelevanceFeedbackExpander prf = new PseudoRelevanceFeedbackExpander(searcher, 3)) {
//      expand(expanders, queries, topics, queryRepresentations, () -> prf), "E6:NONE");
//    }
